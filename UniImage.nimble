...
task coverage, "LCOV + HTML coverage report for the Nim sources (needs lcov)":
  ...
  exec "lcov --capture --directory " & cache & " --base-directory ." &
       " --include \"*/src/UniImage/*\" --output-file lcov.info --quiet --ignore-errors mismatch" &
  ...
  rmDir capiCache
  exec "nim c --path:src --app:staticlib -d:noAutoInit --noMain --mm:arc -d:release --debugger:native" &
       " --passC:--coverage --passL:--coverage --nimcache:" & capiCache &
       " -o:build/libUniImage_cov.a src/UniImage/c_api.nim"
  ...
  exec makeExe & " -C tests/c BIN=test_capi_cov" &
       " LIB=../../build/libUniImage_cov.a EXTRA_CFLAGS=--coverage"
  ...
  exec "lcov --capture --no-function-coverage --directory " & capiCache &
       " --base-directory ." &
       " --include \"*/src/UniImage/*\" --output-file build/capi.info --quiet" &
